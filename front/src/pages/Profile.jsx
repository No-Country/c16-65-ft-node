...
const Profile = () => {
    ...
    const fetchUserPurchases = async (email) => {
        try {
            const response = await fetch(`https://no-country-cwv9.onrender.com/api/purchases/search/${email}`);
            if (response.ok) {
            ...
            <div className="bg-gray-300 p-6 rounded-lg shadow-md">
                ...
                {userData && (
                    <div className='flex flex-col items-center'>
                        <img src={userData.picture} alt={userData.name} className="rounded-full h-24 w-24 mb-4 mx-auto" />
                ...
                <div className="grid gap-4 flex">
                    {userPurchases.map((purchase) => (
                        <div key={purchase._id} className="bg-white p-4 rounded-lg shadow-md">
                            <p>Fecha de compra: {new Date(purchase.purchase_datetime).toLocaleString()}</p>
                            <p>Productos:</p>
                            <ul>
